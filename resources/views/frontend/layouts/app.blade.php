...
<!DOCTYPE html>
@if(optional(\App\Models\Language::where('code', Session::get('locale', Config::get('app.locale')))->first())->rtl == 1)
...
<head>
    ...
    <!-- CSS -->
    <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&family=Cormorant+Garamond:ital,wght@0,400;0,500;0,600;0,700;1,400&display=swap">
    ...
    <style>
        ...
        :root {
            --primary: {{ get_setting('base_color', '#0F1DDB') }};
            --hov-primary: {{ get_setting('base_hov_color', '#0B1488') }};
            --soft-primary: {{ hex2rgba(get_setting('base_hov_color', '#0F1DDB'), .15) }};
            --secondary: {{ get_setting('secondary_color', '#C9A227') }};
            --soft-secondary: {{ hex2rgba(get_setting('secondary_color', '#C9A227'), .15) }};
            --premium-gold: #C9A227;
            --brand-navy: #0A0F3D;
        }

        .text-primary-grad {
            background: rgb(15, 29, 219);
            background: -moz-linear-gradient(0deg, {{ hex2rgba(get_setting('base_color', '#0F1DDB'), 1) }} 0%, {{ hex2rgba(get_setting('secondary_color', '#C9A227'), 1) }} 100%);
            background: -webkit-linear-gradient(0deg, {{ hex2rgba(get_setting('base_color', '#0F1DDB'), 1) }} 0%, {{ hex2rgba(get_setting('secondary_color', '#C9A227'), 1) }} 100%);
            background: linear-gradient(0deg, {{ hex2rgba(get_setting('base_color', '#0F1DDB'), 1) }} 0%, {{ hex2rgba(get_setting('secondary_color', '#C9A227'), 1) }} 100%);
        ...
        .bg-primary-grad {
            background: rgb(15, 29, 219);
            background: -moz-linear-gradient(225deg, {{ hex2rgba(get_setting('base_color', '#0F1DDB'), 1) }} 0%, {{ hex2rgba(get_setting('base_hov_color', '#0B1488'), 1) }} 100%);
            background: -webkit-linear-gradient(225deg, {{ hex2rgba(get_setting('base_color', '#0F1DDB'), 1) }} 0%, {{ hex2rgba(get_setting('base_hov_color', '#0B1488'), 1) }} 100%);
            background: linear-gradient(225deg, {{ hex2rgba(get_setting('base_color', '#0F1DDB'), 1) }} 0%, {{ hex2rgba(get_setting('base_hov_color', '#0B1488'), 1) }} 100%);
            border-radius: 9999px;
        }

        .btn-gold {
            background: linear-gradient(225deg, #C9A227 0%, #A47E1A 100%);
            border-radius: 9999px;
            color: #fff;
        }

        .btn-gold:hover {
            background: linear-gradient(225deg, #A47E1A 0%, #8B6914 100%);
            color: #fff;
        }
        ...
        .hero-gradient {
            background: linear-gradient(135deg, #0A0F3D 0%, #1A1A6E 30%, #0F1DDB 60%, #0B1488 100%);
        }
        ...
        .fill-primary-grad stop:nth-child(1) {
            stop-color: {{ hex2rgba(get_setting('secondary_color', '#C9A227'), 1) }};
        }

        .fill-primary-grad stop:nth-child(2) {
            stop-color: {{ hex2rgba(get_setting('base_color', '#0F1DDB'), 1) }};
        }
    </style>
