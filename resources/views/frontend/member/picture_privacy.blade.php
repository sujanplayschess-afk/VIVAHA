          ...
          <form action="{{ route('member.update_picture_privacy', Auth::user()->id) }}" method="POST">
              ...
              <div class="card shadow-none border mb-4">
                  <div class="card-header bg-soft-primary py-2">
                      <h6 class="mb-0 fs-14 fw-700 text-primary">{{ translate('Premium Privacy Features') }}</h6>
                  </div>
                  <div class="card-body">
                      <div class="form-group row align-items-center mb-4">
                          <div class="col-md-9">
                              <label class="mb-0 fs-14 fw-600">{{ translate('Total Profile Invisibility') }}</label>
                              <small class="d-block text-muted">{{ translate('Your profile remains hidden from public search and casual users.') }}</small>
                          </div>
                          <div class="col-md-3 text-right">
                              <label class="aiz-switch aiz-switch-success mb-0">
                                  <input type="checkbox" name="is_invisible" value="1" @if(Auth::user()->is_invisible) checked @endif>
                                  <span></span>
                              </label>
                          </div>
                      </div>
                      <div class="form-group row align-items-center mb-0">
                          <div class="col-md-9">
                              <label class="mb-0 fs-14 fw-600">{{ translate('On-Demand Photo & Bio Access') }}</label>
                              <small class="d-block text-muted">{{ translate('Retain total control over who can view your identity on a case-by-case basis.') }}</small>
                          </div>
                          <div class="col-md-3 text-right">
                              <label class="aiz-switch aiz-switch-success mb-0">
                                  <input type="checkbox" name="require_photo_permission" value="1" @if(Auth::user()->require_photo_permission) checked @endif>
                                  <span></span>
                              </label>
                          </div>
                      </div>
