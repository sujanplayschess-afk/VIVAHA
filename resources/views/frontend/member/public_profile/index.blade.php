                ...
                <div class="px-3 row align-items-center">
                    ...
                    <h1 class="fs-24 fw-600">
                        ...
                        @if($user->documents_verified_at)
                            <i class="fa-solid fa-circle-check text-gold ms-1" title="{{ translate('Verified Member') }}"></i>
                        @endif
                        @if($user->priority_badge)
                            <span class="badge badge-inline badge-warning text-white fs-12 ml-2"><i class="fa-solid fa-crown me-1"></i>{{ translate('Priority') }}</span>
                        @endif
    ...
    <div class="container">
        <div class="row">
                    ...
                    <div class="overflow-hidden rounded shadow-lg mb-4 bg-white d-none d-xl-block">
                        @php
                            ...
                            if(Auth::check()){
                                if(Auth::user()->id == $user->id) {
                                    ...
                                    $permission = \App\Models\PhotoPermission::where('user_id', $user->id)->where('sender_id', Auth::user()->id)->where('status', 1)->first();
                                    if($permission) {
                                        $photo_access = true;
                                    }
                                }
                            ...
                            if($user->require_photo_permission == 1 && !$photo_access){
                                $profile_picture_show = 'locked';
                            }
                        ...
                        <div class="position-relative">
                            <img
                                ...
                                @elseif($profile_picture_show == 'locked')
                                    src="{{ uploaded_asset($user->photo) }}" style="filter: blur(15px);"
                                @else
                            ...
                            >
                            @if($profile_picture_show == 'locked')
                                <div class="absolute-full d-flex align-items-center justify-content-center bg-dark-60">
                                    <div class="text-center">
                                        <i class="las la-lock text-white la-3x mb-2"></i>
                                        @php
                                            $request_sent = \App\Models\PhotoPermission::where('user_id', $user->id)->where('sender_id', Auth::user()->id)->first();
                                        @endphp
                                        @if(!$request_sent)
                                            <form action="{{ route('member.photo_view_request') }}" method="POST">
                                                @csrf
                                                <input type="hidden" name="id" value="{{ $user->id }}">
                                                <button type="submit" class="btn btn-primary btn-sm">{{ translate('Request Photo Access') }}</button>
                                            </form>
                                        @else
                                            <button class="btn btn-secondary btn-sm" disabled>
                                                @if($request_sent->status == 0)
                                                    {{ translate('Request Pending') }}
                                                @else
            ...
            <div class="col-xl-8 offset-xxl-1">
                ...
                <div class="tab-content no-screenshot">
